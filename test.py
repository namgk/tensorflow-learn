...
print('Model restored from ' + LOGDIR + '/' + args.saved_model)

print("test accuracy %g"%accuracy.eval(session=sess, feed_dict={
    x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1.0}))
  
result = y_conv.eval(session=sess, feed_dict={
    x: mnist.test.images[:1], keep_prob: 1.0})

print(tf.argmax(result,1).eval(session=sess))
print(tf.argmax(mnist.test.labels[:1],1).eval(session=sess))
